...
def realizarPrueba():
    ...
    if (flag != False):
        ...
        if(F > FF):
            print(f"{bcolors.WARNING}Se rechaza la hipotesis nula, se acepta la hipotesis alterna{bcolors.ENDC}")
        else:
            print(f"{bcolors.OKGREEN}Se acepta la hipotesis nula y se rechaza la hipotesis alterna{bcolors.ENDC}")
    else:
