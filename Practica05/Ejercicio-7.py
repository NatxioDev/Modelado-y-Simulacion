...
beta0 = ((yy * x2).sum() - (getCos(x2) * x2).sum()) / \
    ((getSin(x1).sum() * (getSin(x2) * x2).sum() + (getSin(x1) * x2).sum()))

beta1 = (-(yy * x2).sum() + ((beta0 * getSin(x1) * x2).sum() +
                             (getCos(x2) * x2).sum()) / (getSin(x2) * x2).sum())
...
def realizarPrueba():
    ...
    print(f"{bcolors.HEADER}Ejercicio 7: \n{bcolors.ENDC}")
    print(
    ...
    # Desviaciones
    # SqrtS2_Matriz = math.sqrt(S2_Total) * math.sqrt(C)
